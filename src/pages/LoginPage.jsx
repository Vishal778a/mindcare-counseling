import { useEffect, useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { useAuth } from '../hooks/useAuth';

export default function LoginPage() {
  const { loginAsPatient, isAuthenticated, isAdmin } = useAuth();
  const navigate = useNavigate();
  const [enrolling, setEnrolling] = useState(false);

  // Redirect if already logged in
  useEffect(() => {
    if (isAuthenticated) {
      navigate(isAdmin ? '/dashboard' : '/patient/dashboard', { replace: true });
    }
  }, [isAuthenticated, isAdmin, navigate]);

  const handlePatientLogin = () => {
    loginAsPatient();
    navigate('/patient/dashboard');
  };

  if (isAuthenticated) return null;

  return (
    <div className="login-page animate-fade-in">
      <div className="card card-glass login-card animate-scale-in">
        <div className="login-icon">🧠</div>
        <h1>Welcome to <span className="text-gradient">MindCare</span></h1>
        <p>Sign in to book sessions and manage your mental health journey.</p>

        {/* Patient Google Sign-In (when configured) */}
        {/* For now, demo patient login */}
        <button
          className="google-btn"
          onClick={handlePatientLogin}
          id="patient-login-btn"
        >
          <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          Continue with Google
        </button>

        <p style={{
          marginTop: 'var(--space-6)',
          fontSize: 'var(--text-xs)',
          color: 'var(--color-text-tertiary)',
        }}>
          By signing in, you agree to our Terms of Service and Privacy Policy.
          <br />Your data is encrypted and confidential.
        </p>

        <div className="divider" style={{ margin: 'var(--space-4) 0' }}></div>

        <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-tertiary)' }}>
          New here?{' '}
          <Link to="/patient/enroll" style={{ color: 'var(--color-accent-secondary)' }}>
            Enroll as a patient →
          </Link>
        </p>
      </div>
    </div>
  );
}
